...
export default function LoginPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const [scenePhase, setScenePhase] = useState<"idle" | "accelerating" | "collapsed">("idle");

  async function signInWithGoogle() {
    setScenePhase("accelerating");
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-sm rounded-lg border border-border bg-surface p-6 animate-fade-up">
          ...
          <div className="space-y-3">
            ...
            {mode === "signIn" && (
              <button onClick={() => { setMode("forgot"); setError(null); setInfo(null); }} className="text-xs text-muted hover:text-gold">
                {t("forgotPassword")}
              </button>
            )}
